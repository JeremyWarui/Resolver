...
import { ChevronLeft, MessageSquare, ArrowUpCircle, Loader2, CheckCircle2, Clock, AlertTriangle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';
import { useMobileTicketDetail, useUpdateTicketStatus, useAddComment } from './useMobileTickets';
...
function StatusActions({ ticketId, status, isOnline }: StatusActionProps) {
  ...
  const handleAction = (next: string) => {
    if (!isOnline) {
      toast.error('You are offline. Action queued and will sync when connection returns.');
      return;
    }
    if (next === 'pending') {
      setReasonFor(next);
  ...
  const submitWithReason = () => {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          toast.success('Ticket put on hold');
          setReasonFor(null);
          setReason('');
        },
...
function CommentBox({ ticketId, isOnline }: CommentBoxProps) {
  ...
  const submit = () => {
    if (!body.trim()) return;
    if (!isOnline) {
      toast.error('You are offline. Comment queued.');
      return;
    }
    mutate(body.trim(), {
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-border px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-semibold truncate">{t.service_item?.name ?? t.description}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-border divide-y divide-border/70">
          {[
            ...
            { label: 'Raised by', value: (t.raised_by as unknown as { name?: string; username?: string })?.name ?? (t.raised_by as unknown as { username?: string })?.username ?? '—' },
        ...
        {t.current_level && t.current_level !== 'technician' && (
          <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
            ...
            <p className="text-xs text-red-700 font-medium">
              This ticket has been escalated (level {t.current_level ?? 1})
            </p>
          </div>
        )}
